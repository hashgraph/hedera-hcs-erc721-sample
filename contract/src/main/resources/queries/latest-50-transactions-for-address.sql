select distinct on (t.timestamp) t.timestamp,
                                 ...
                                 case t.function
                                     ...
                                     when 5 then (
                                         select jsonb_build_object('spender', encode(spender.address, 'hex'), 'id',
                                                                   '' || ta.token_id)
                                     ...
                                     when 6 then (
                                         select jsonb_build_object('operator', encode(operator_.address, 'hex'),
                                                                   'approved', tsafa.approved)
                                     ...
                                     when 7 then (
                                         select jsonb_build_object('to', encode(to_.address, 'hex'), 'id',
                                                                   '' || tm.token_id)
                                     ...
                                     when 9 then (
                                         select jsonb_build_object('from', encode(from_.address, 'hex'), 'to',
                                                                   encode(to_.address, 'hex'), 'id', '' || ttf.token_id)
...
where at.address_id = (select a.id from address a where a.address = decode($1, 'hex'))
